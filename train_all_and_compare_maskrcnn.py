...
def train_and_evaluate(model_name, model, output_folder):
    ...
    Args:
        model_name (str): A name identifier, e.g. 'retinanet', 'fasterrcnn', etc.
        model (nn.Module): A TorchVision detection model (RetinaNet, Faster R-CNN, etc.)
...
# C) Mask R-CNN (ResNet50-FPN)
maskrcnn = maskrcnn_resnet50_fpn(weights=MaskRCNN_ResNet50_FPN_Weights.DEFAULT)
# Adjust num_classes in the box_predictor
in_features_m = maskrcnn.roi_heads.box_predictor.cls_score.in_features
maskrcnn.roi_heads.box_predictor = torchvision.models.detection.faster_rcnn.FastRCNNPredictor(
    in_features_m, num_classes
)
# Also need to set mask_predictor if you have more than 1 foreground class
# by default, maskrcnn is set up for 91 classes if using COCO weights
in_features_mask = maskrcnn.roi_heads.mask_predictor.conv5_mask.in_channels
maskrcnn.roi_heads.mask_predictor = torchvision.models.detection.mask_rcnn.MaskRCNNPredictor(
    in_features_mask, 256, num_classes
)
maskrcnn.to(device)

res_mask = train_and_evaluate(
    model_name="maskrcnn",
    model=maskrcnn,
    output_folder=os.path.join(output_parent, "maskrcnn")
)
all_results.append(res_mask)
